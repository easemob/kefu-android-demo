...
public class RatingBar extends LinearLayout {
    ...
    public RatingBar(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < starCount; ++i) {
            final ImageView imageView = getStarImageView(i, starCount);
     ...
     * 设置每颗星星的参数
     */
    private ImageView getStarImageView(int index, int num) {
        ImageView imageView = new ImageView(getContext());

        LayoutParams layout = new LayoutParams(
                Math.round(starImageSize), Math.round(starImageSize));//设置每颗星星在线性布局的大小
        if (num > 1 && index != 0){
            layout.setMargins(Math.round(starPadding), 0, 0, 0);//设置每颗星星在线性布局的间距
        }
